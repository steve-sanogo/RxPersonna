...
class NEREngine:
    ...
    def supprimer_titres(nom):
        titres = ['mr', 'mme', 'monsieur', 'madame', 'm.', 'dr', 'sir', 'miss', 'ms', 'mademoiselle',
    ...
    def sont_lies(nom1, nom2):
        ...
        # S'il n'y a rien en commun, c'est fini
        if not commun:
            return False

        # --- SECURITE FAMILLE ---
        # Si les deux noms sont composés (ex: "Hari Seldon" et "Dors Seldon")
        if len(mots1) > 1 and len(mots2) > 1:
            prenom1 = n1.split()[0]
            prenom2 = n2.split()[0]

            # On vérifie les initiales compatibles
            compatible_initiale = (
            ...
            # Si prénoms différents ET pas d'initiale compatible -> On refuse
    ...
    def resolve_aliases(noms, save_path):
        # Ta logique de regroupement avec la fonction 'sont_lies' Hybride
        groupes = []
        ...
        for nom in noms_tries:
            ...
            # Ici, le tri pour mettre les noms sans titre d'abord, ceux avec titre à la fin
            groupe_sans_titres = [nom for nom in groupe if NEREngine.supprimer_titres(nom) == nom]
            groupe_avec_titres = [nom for nom in groupe if NEREngine.supprimer_titres(nom) != nom]
    ...
    def process_entities(entities, book, chapter):
        ...
        return NEREngine.resolve_aliases(noms=per_entities_list, save_path=doc_path)